...
import { Loader2, Phone, User, Bike, MapPin, Calendar, Users, Mail, CheckCircle, XCircle, AlertCircle, Briefcase, Clock, Upload, X } from "lucide-react";
...
const ImageUploader = ({ label, name, value, onChange, preview, onRemove }) => {
...
export default function PartnerSignupForm() {
  ...
  const handleImageChange = (fieldName, url) => {
    ...
    setPreviews({ ...previews, [fieldName]: url });
  };
  ...
  const handleSubmit = async () => {
    if (!form.full_name || !form.phone_number) {
      showToast("Please fill in required fields: Full Name and Phone Number", "error");
      return;
    }

    setLoading(true);
    try {
      const res = await api.post("/partner/signup", form);
      ...
      <div className="min-h-screen bg-gray-100 sm:py-12">
        <div className="w-full max-w-7xl mx-auto">
          <div className="bg-white rounded-2xl overflow-hidden">
            <div className="px-6 py-8 sm:px-8 sm:py-10 max-h-[calc(100vh-140px)] overflow-y-auto">
              <h1 className="text-3xl font-bold text-gray-900 mb-8">Partner Registration Form</h1>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <ImageUploader 
                        ...
                        value={form.driving_license_front} 
                        onChange={handleImageChange} 
                        preview={previews.driving_license_front}
                        onRemove={handleRemoveImage}
                      />
                      <ImageUploader 
                        label="Driving License Back" 
                        name="driving_license_back" 
                        value={form.driving_license_back} 
                        onChange={handleImageChange} 
                        preview={previews.driving_license_back}
                        onRemove={handleRemoveImage}
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input 
                          ...
                          onChange={handleChange} 
                          className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:border-yellow-500 focus:ring-2 focus:ring-yellow-200 outline-none text-gray-900 placeholder-gray-400 transition-all" 
                        />
                      </div>
                      <div>
